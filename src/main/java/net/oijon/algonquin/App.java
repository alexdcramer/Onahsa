...
public class App {
	/**
	 * Gets the name of files for use in createAudio
	 * 
	 * @param input The string you want .wav files from
	 * @return The array of your .wav file names (without the .wav)
	 */
	public static String[] getFileNames(String input) {
		...
		// g and ɡ¡ are the same sound, however two different points in unicode. as
		// such, they need to both be in there to prevent disappearing chars
		char[] ipaList = { 'p', 'b', 't', 'd', 'ʈ', 'ɖ', 'c', 'ɟ', 'k', 'g', 'ɡ', 'q', 'ɢ', 'ʔ', 'm', 'ɱ', 'n', 'ɳ',
				'ɲ', 'ŋ', 'ɴ', 'ʙ', 'r', 'ʀ', 'ⱱ', 'ɾ', 'ɽ', 'ɸ', 'β', 'f', 'v', 'θ', 'ð', 's', 'z', 'ʃ', 'ʒ', 'ʂ', 'ʐ',
				'ç', 'ʝ', 'x', 'ɣ', 'χ', 'ʁ', 'ħ', 'ʕ', 'h', 'ɦ', 'ɬ', 'ɮ', 'ʋ', 'ɹ', 'ɻ', 'j', 'ɰ', 'l', 'ɭ', 'ʎ', 'ʟ',
				'ʍ', 'w', 'ɥ', 'ʜ', 'ʢ', 'ʡ', 'ɕ', 'ʑ', 'ɺ', 'ɧ', 'i', 'y', 'ɨ', 'ʉ', 'ɯ', 'u', 'ɪ', 'ʏ', 'ʊ', 'e', 'ø',
				'ɘ', 'ɵ', 'ɤ', 'o', 'ə', 'ɛ', 'œ', 'ɜ', 'ɞ', 'ʌ', 'ɔ', 'æ', 'ɐ', 'a', 'ɶ', 'ɑ', 'ɒ' };
		char[] diacriticList = { '̥', '̬', '̹', '̜', '̟', '̠', '̈', '̽', '̩', '̯', '˞', '̤', '̰', '̼', 'ʷ', 'ʲ', 'ˠ',
				'ˤ', '̴', '̝', '̞', '̘', '̙', '̪', '̺', '̻', '̃', 'ⁿ', 'ˡ', '̚' };
		...
		for (int i = 0; i < inputLength; i++) {
			...
			if (isDiacritic != true) {
				for (int k = 0; k < ipaList.length; k++) {
					if (c == ipaList[k]) {
						...
						fileNames[currentFileName] = "" + c; // this is stupid but otherwise i cant cast from char to
																// string
						currentFileName++;
					}
				}
			...
			// i am very much hoping that there is some way to simply edit the .wav files in
			// batch
	...
	/*
	 * Plays audio from file names
	 * 
	 * @param fileNames Used to get the names of the files to play
	 */
